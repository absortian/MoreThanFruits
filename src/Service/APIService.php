...
class APIService extends AbstractController
{
    ...
    public function __construct(EventDispatcherInterface $eventDispatcher, UrlGeneratorInterface $router, UserService $userService, FruitOrderService $fruitOrderService, FruitService $fruitService){
    ...
    /**
     * Función que crea un pedido en la plataforma
     */
    public function apiCreateOrder($data){
        // Respuesta por defecto
        $response = array(
            "status" => false,
            "message" => "Error inesperado, prueba más tarde."
        ...
        if(isset($authUser["status"]) && $authUser["status"]){
            ...
            $data["user"] = $user;
            if($user instanceof User && in_array("ROLE_ADMIN",$user->getRoles())){
                if(isset($data["providerName"]) && isset($data["countryCode"]) && isset($data["phone"]) && isset($data["providerId"]) && isset($data["fruitName"]) && isset($data["amount"])){
                    // Creamos el pedido
                    $createOrderResponse = $this->fruitOrderService->createFruitOrder($data["providerName"], $data["countryCode"], $data["phone"], $data["providerId"], $data["fruitName"], $data["amount"]);
                    if(isset($createOrderResponse["status"]) && !$createOrderResponse["status"]){
    ...
    /**
     * Función que elimina una fruta en la plataforma
     */
    public function apiDeleteFruit($data){
        // Respuesta por defecto
        $response = array(
            "status" => false,
            "message" => "Error inesperado, prueba más tarde."
        ...
        if(isset($authUser["status"]) && $authUser["status"]){
            ...
            $data["user"] = $user;
            if($user instanceof User && in_array("ROLE_ADMIN",$user->getRoles())){
                if(isset($data["fruitName"])){
                    // Creamos la fruta
                    $deleteFruitResponse = $this->fruitService->deleteFruit($data["fruitName"]);
                    if(isset($deleteFruitResponse["status"]) && !$deleteFruitResponse["status"]){
                        $response = $deleteFruitResponse;
                    }else{
                        $response = array(
                            "status" => true,
    ...
    /**
     * Función que crea una fruta en la plataforma
     */
    public function apiCreateFruit($data){
        // Respuesta por defecto
        $response = array(
            "status" => false,
            "message" => "Error inesperado, prueba más tarde."
        ...
        if(isset($authUser["status"]) && $authUser["status"]){
            ...
            if($user instanceof User && in_array("ROLE_ADMIN",$user->getRoles())){
                if(isset($data["fruitName"])){
                    // Creamos la fruta
                    $createFruitResponse = $this->fruitService->createFruit($data["fruitName"]);
                    if(isset($createFruitResponse["status"]) && !$createFruitResponse["status"]){
                        $response = $createFruitResponse;
                    }else{
                        $response = array(
                            "status" => true,
                            "message" => "Fruta creada."
                        );
                    }
                }else{
                    $response = array(
    ...
    /**
     * Función que obtiene las frutas de la plataforma
     */
    public function apiGetFruits($data){
        // Respuesta por defecto
        $response = array(
            "status" => false,
            "message" => "Error inesperado, prueba más tarde."
        ...
        if(isset($authUser["status"]) && $authUser["status"]){
            ...
            if($user instanceof User && in_array("ROLE_ADMIN",$user->getRoles())){
                // Obtengo em
                $entityManager = $this->getDoctrine()->getManager();
                // Obtengo el repositorio de orders
                $fruitRepo = $entityManager->getRepository("App:Fruit");
                if(isset($data["datatable"]) && $data["datatable"]){
                    $dataResponse = $fruitRepo->getFruitsDataTable($data);
                    $response = $dataResponse;
                }else{
                    $dataResponse = $fruitRepo->getFruits($data);
                    $response = array(
                        "status" => true,
                        "data" => $dataResponse
                    );
    ...
    /**
     * Función que obtiene los pedidos de la plataforma
     */
    public function apiGetOrders($data){
        // Respuesta por defecto
        $response = array(
            "status" => false,
            "message" => "Error inesperado, prueba más tarde."
        ...
        if(isset($authUser["status"]) && $authUser["status"]){
            ...
            $entityManager = $this->getDoctrine()->getManager();
            // Obtengo el repositorio de orders
            $fruitOrderRepo = $entityManager->getRepository("App:FruitOrder");
            if(isset($data["datatable"]) && $data["datatable"]){
                $dataResponse = $fruitOrderRepo->getOrdersDataTable($data);
                $response = $dataResponse;
            }else{
                $dataResponse = $fruitOrderRepo->getOrders($data);
                $response = array(
                    "status" => true,
                    "data" => $dataResponse
                );
